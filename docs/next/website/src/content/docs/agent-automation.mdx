...
`--kind` selects a supported agent and its canonical executable. Supported kinds are `pi`, `pii`, `prime-agent`, `claude`, `codex`, `gemini`, `cursor`, `devin`, `agy`, `cline`, `omp`, `mastracode`, `opencode`, `copilot`, `kimi`, `kiro`, `droid`, `amp`, `grok`, `hermes`, `kilo`, `qodercli`, `qwen`, `maki`, `muse`, and `rusty`. Arguments after `--` are passed unchanged to that executable.
