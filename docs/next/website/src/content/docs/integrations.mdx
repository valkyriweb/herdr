---
title: Integrations
description: Install Herdr integrations for Pi (including the pii fork launcher), OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Cursor Agent CLI, MastraCode, Antigravity CLI, and Grok CLI.
---
...
| --- | --- | --- |
| Lifecycle authority | Pi (including `pii`), OMP, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
Some integrations report native agent session references. Herdr uses official session references to resume Claude Code, Codex, Devin CLI, Droid, Kimi Code CLI, Qoder CLI, Qwen Code, Cursor Agent CLI, Grok CLI, GitHub Copilot CLI, Pi (including the `pii` fork launcher), Prime Agent, OMP, Rusty, Hermes Agent, OpenCode, Kilo Code CLI, MastraCode, and Antigravity CLI panes after a Herdr server restart unless `[session] resume_agents_on_restore = false` disables it.

Native session restore requires current Herdr integrations: Pi (including `pii`) integration version `11`, OMP version `3`, Claude Code version `6`, Codex version `5`, GitHub Copilot CLI version `2`, Devin CLI version `2`, Droid version `2`, Kimi Code CLI version `3`, Qoder CLI version `2`, Qwen Code version `1`, Cursor Agent CLI version `1`, Grok CLI version `1`, OpenCode version `5`, Kilo Code CLI version `1`, Hermes Agent version `5`, MastraCode version `1`, or Antigravity CLI version `1`. Check installed versions with `herdr integration status`.

## Pi and pii

The same integration supports vanilla `pi` and the customized `pii` launcher. It reports `pi` for vanilla Pi and `pii` when the `pii` launcher sets `HERDR_PI_VARIANT=pii`; the vanilla launcher clears that marker so inherited fork state cannot misidentify `pi`. Install the shared integration once in the agent profile used by the launchers.
...
Prime Agent ships its own Herdr reporter, so there is nothing to install. Because it is built on Pi, the reporter uses the `herdr:pi` source with the `prime-agent` agent label and reports lifecycle state plus the session file path (falling back to the session id) on every state change. Herdr accepts that pair as an official integration, detects the `prime-agent` process, and resumes a Prime Agent pane with `prime-agent --resume <session>` after a server restart.

## Rusty

Rusty's `rusty` launcher starts a reporter beside each session when it runs inside a Herdr pane, so there is nothing to install. The reporter uses the `herdr:rusty` source with the `rusty` agent label and reports lifecycle state together with the durable session name as the session id. Herdr accepts that pair as an official integration, detects the `rusty` launcher as the pane's foreground process, and resumes a Rusty pane with `rusty --resume <session>` after a server restart. Launchers that do not pass a session id still report state; only session restore stays unavailable for them.
